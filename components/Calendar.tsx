...
export default function Calendar(props: Props) {
    ...
    for (const day of days) {
      for (let hour = earliest; hour < latest; hour++) {
        for (let minute = 0; minute < 60; minute += 15) {
          ...
          let mmt = moment.tz(
            `2023-01-${dayPadded} ${hourPadded}:${minutePadded}:00`,
            timezone
          )
  ...
  useEffect(() => {
    ...
    for (const date of newDates) {
      ...
      for (let hour = minHour; hour <= maxHour; hour++) {
        ...
        const activeHourExists = activeIntervals.some(
          (i) => parseHour(i) === hour
        )
    ...
    for (let hour = minHour; hour <= maxHour; hour++) {
      ...
      const fallBack = newDays.some(
        (d) => d.intervals.filter((i) => i.hour === hour).length > 4
      )
  ...
  function intervalSelected(dayIndex: number, intIndex: number, index: number) {
    ...
    if (!dragAdd && dragStart && dragEnd) {
      ...
      if (
        ...
        intIndex >= minIntIndex &&
        intIndex <= maxIntIndex
      )
        return false
    }
    // if already selected
  ...
  const finishDrag = useCallback(() => {
    ...
    for (let dayIndex = minDayIndex; dayIndex <= maxDayIndex; dayIndex++) {
      for (let intIndex = minIntIndex; intIndex <= maxIntIndex; intIndex++) {
        ...
        if (dragAdd && indexIndex === -1)
          newSelectedIndices.push(interval.index)
        if (!dragAdd && indexIndex !== -1)
          newSelectedIndices.splice(indexIndex, 1)
      }
    }
  ...
  function intervalFaded(interval: Interval) {
    ...
    if (
      ...
      hoveredRespondent === null &&
      hoveredShade === null
    )
      return false
    const currRespondents = respondents.filter(
      (r) => selectedRespondents.includes(r.id) || r.id === hoveredRespondent
    )
    const { index, active } = interval
    if (!active) return true
    if (hoveredShade !== null && getShade(interval.index) !== hoveredShade)
      return true
              ...
              {day.intervals.map((interval, j) => (
                <div
                  ...
                  className={styleBuilder([
                    ...
                    [
                      styles.faded,
                      type === 'display' && intervalFaded(interval),
                    ],
                    [
                      styles.gapped,
                      j > 0 &&
                        j % 4 === 0 &&
                        day.intervals[j].hour !== day.intervals[j - 1].hour &&
                        day.intervals[j].hour - 1 !== day.intervals[j - 1].hour,
                    ],
                  ])}
                  style={
